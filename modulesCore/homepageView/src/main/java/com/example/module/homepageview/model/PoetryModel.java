...
public class PoetryModel implements IPoetryContract.IPoetryModel{
    ...
    public PoetryModel(Context mContext) {
        this.mContext = mContext;
    }

    @Override
    public void getPoetryDatas(PoetryCallback poetryCallback) {
        OkHttpClient okHttpClient = new OkHttpClient();
        String token = TokenManager.getToken(mContext);
        Log.d(TAG, "Token为：" + token);

        Request.Builder builder = new Request.Builder();
        builder.url(POETRY_URL);
        if (token != null) {
        ...
        okHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(@NonNull Call call, @NonNull IOException e) {
                Log.e(TAG, "请求失败", e);
                if (poetryCallback != null) {
                    poetryCallback.onFailure(e);
                }
            }
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                if (!response.isSuccessful()) {
                    ...
                    if (poetryCallback != null) {
                        poetryCallback.onFailure(new IOException("请求失败，状态码：" + response.code()));
                    }
                    return;
                }
                ...
                if (body == null) {
                    Log.e(TAG, "响应体为空");
                    if (poetryCallback != null) {
                        poetryCallback.onFailure(new IOException("响应体为空"));
                    }
                    return;
                }
                ...
                Gson gson = new Gson();
                Poetry poetry = gson.fromJson(responseBody, Poetry.class);
                Log.d(TAG, "解析后的数据：" + poetry);

                // 确保回调在主线程中执行
                if (poetryCallback != null) {
                    new Handler(Looper.getMainLooper()).post(() -> poetryCallback.onSuccess(poetry.getData()));
                }
            }
